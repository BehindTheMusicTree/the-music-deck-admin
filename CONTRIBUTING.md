...
| Variable | **`TMD_ADMIN_API_APP_NAME`**                                                      | Image name under **`ghcr.io/<GHCR_IMAGE_NAMESPACE>/…`**. **`api-release`** publishes **`TMD_ADMIN_API_TAG`** into pooled **`tmd-admin-<env>.tags.env`** via **`set-image-tags-on-server`**. On the server, **`TMD_ADMIN_API_IMAGE`** in **`scripts/.env`** is the repo without tag — not a GitHub variable. |
...
| Variable | **`IMAGE_TAGS_POOL_DIR`**                                                         | **Required** for **`api-release`**: pooled manifests directory on the VPS (e.g. **`/srv/btmt/image-tags`**). Same variable as **BehindTheMusicTree/infrastructure** / [**github-workflows** **`set-image-tags-on-server`**](https://github.com/BehindTheMusicTree/github-workflows/blob/main/README.md#set-image-tags-on-server). |
| Variable | **`ENV_POOL_DIR`**                                                                | **Required** for **Sync env**: pooled fragment directory (e.g. **`/srv/btmt/env-pool`**). Same as infrastructure **`ENV_POOL_DIR`** / **`sync-env-to-server`**. |
...
| Secret   | **`SERVER_DEPLOY_USERNAME`**, **`SERVER_DEPLOY_SSH_PRIVATE_KEY`**                 | Deploy user SSH (required by **`set-image-tags-on-server`**, **`sync-env-to-server`**, and other **github-workflows** jobs that SSH to the VPS). |
...
[`api-release.yml`](.github/workflows/api-release.yml) pins [**github-workflows**](https://github.com/BehindTheMusicTree/github-workflows) **`@v2.0.0`**: **`set-image-tags-on-server.yml`** (**`stack: tmd-admin`**, **`TMD_ADMIN_API_TAG`**) and **`call-redeployment-webhook.yml`**. Bump the tag together when adopting a newer release. Webhook success bodies must start with **`Redeployment accepted`** once infrastructure **`generate-hooks-json.sh`** is deployed (see github-workflows **Expected Webhook Response**).
